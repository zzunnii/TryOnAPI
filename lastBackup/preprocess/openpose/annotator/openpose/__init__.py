...
class OpenposeDetector:
    def __init__(self):
        ...
        body_modelpath = settings.OPENPOSE_BODY_MODEL
        ...
        if not os.path.exists(body_modelpath):
            try:
                ...
                print(f"Looking for OpenPose model at: {body_modelpath}")
                print(f"If not found, will try to download from HuggingFace")
                
                from basicsr.utils.download_util import load_file_from_url
                load_file_from_url(body_model_path, model_dir=annotator_ckpts_path)
            except ImportError as e:
                print(f"Error importing settings or downloading models: {e}")
                raise RuntimeError(f"Could not find body pose model at {body_modelpath} and failed to download it")
