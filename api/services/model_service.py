    ...
    def generate_tryon_image(
        ...
        try:
            ...
            try:
                ...
                # DensePose 이미지 로드 부분 수정 - Base64 데이터 처리 추가
                if isinstance(pose_image_path, dict) and "visualization_base64" in pose_image_path:
                    # Base64 데이터가 직접 전달된 경우
                    import base64
                    from io import BytesIO

                    img_data = base64.b64decode(pose_image_path["visualization_base64"])
                    pose_img_original = Image.open(BytesIO(img_data)).convert("RGB")
                elif isinstance(pose_image_path, dict) and "densepose_data" in pose_image_path:
                    # DensePose API 응답 객체가 전달된 경우
                    densepose_data = pose_image_path["densepose_data"]
                    if "visualization_base64" in densepose_data:
                        import base64
                        from io import BytesIO

                        img_data = base64.b64decode(densepose_data["visualization_base64"])
                        pose_img_original = Image.open(BytesIO(img_data)).convert("RGB")
                    else:
                        # Base64 데이터가 없으면 경로를 시도
                        pose_img_original = Image.open(pose_image_path.get("visualization_url", pose_image_path)).convert("RGB")
                else:
                    # 기존 방식으로 경로에서 로드 시도
                    pose_img_original = Image.open(pose_image_path).convert("RGB")

            except Exception as e:
                # 로드 실패 시 Linux 경로로 변환 시도
                try:
                    if isinstance(pose_image_path, str) and pose_image_path.startswith("C:"):
                        # Windows 경로를 Linux 경로로 변환 (임시방편)
                        # C:\home\api\output\... -> /home/api/output/...
                        linux_path = pose_image_path.replace("C:", "").replace("\\", "/")
                        pose_img_original = Image.open(linux_path).convert("RGB")
                    else:
                        raise e  # 변환 실패 시 원래 오류 발생
                except:
                    # 모든 시도 실패 시
                    print(f"포즈 이미지 로드 중 오류: {str(e)}")
                    print(f"문제의 경로: {pose_image_path}")
                    return {"status": "error", "message": f"포즈 이미지를 불러올 수 없습니다: {str(e)}"}
