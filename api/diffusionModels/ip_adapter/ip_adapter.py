...
class IPAdapterPlus_Lora(IPAdapter):
    ...
    def set_ip_adapter(self):
        ...
        for attn_processor_name, attn_processor in unet.attn_processors.items():
            ...
            cross_attention_dim = None if attn_processor_name.endswith(
            ...
            if cross_attention_dim is None:
                ...
                attn_procs[attn_processor_name] = IPAttnProcessor2_0_Lora(hidden_size=hidden_size,
                                                                          cross_attention_dim=cross_attention_dim,
                                                                          num_tokens=self.num_tokens)
            ...
            attn_module.q_lora = LoRALinearLayer(in_features=attn_module.to_q.in_features,
                                                 out_features=attn_module.to_q.out_features, rank=self.rank)
            attn_module.k_lora = LoRALinearLayer(in_features=attn_module.to_k.in_features,
                                                 out_features=attn_module.to_k.out_features, rank=self.rank)
            attn_module.v_lora = LoRALinearLayer(in_features=attn_module.to_v.in_features,
                                                 out_features=attn_module.to_v.out_features, rank=self.rank)
            attn_module.out_lora = LoRALinearLayer(in_features=attn_module.to_out[0].in_features,
                                                   out_features=attn_module.to_out[0].out_features, rank=self.rank)
