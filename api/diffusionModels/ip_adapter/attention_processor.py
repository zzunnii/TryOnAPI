...
class IPAttnProcessor2_0_Lora(torch.nn.Module):
    ...
    def __init__(self, hidden_size, cross_attention_dim=None, scale=1.0, scale_lora=1.0, rank=4, num_tokens=4):
        ...
        self.to_k_ip = nn.Linear(cross_attention_dim or hidden_size, hidden_size, bias=False)
        self.to_v_ip = nn.Linear(cross_attention_dim or hidden_size, hidden_size, bias=False)
        self.to_k_ip_lora = LoRALinearLayer(in_features=self.to_k_ip.in_features,
                                            out_features=self.to_k_ip.out_features, rank=rank)
        self.to_v_ip_lora = LoRALinearLayer(in_features=self.to_v_ip.in_features,
                                            out_features=self.to_v_ip.out_features, rank=rank)
