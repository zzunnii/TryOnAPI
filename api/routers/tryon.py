...
async def process_tryon(
    ...
    try:
        ...
        if "visualization_base64" in densepose_result:
            visualization_base64 = densepose_result["visualization_base64"]
        elif "densepose_data" in densepose_result and "visualization_base64" in densepose_result["densepose_data"]:
            visualization_base64 = densepose_result["densepose_data"]["visualization_base64"]

        # Base64 데이터가 있으면 이미지 파일로 저장
        if visualization_base64:
            ...
            with open(visualization_path, "wb") as f:
                f.write(base64.b64decode(visualization_base64))
            print(f"DensePose 시각화 이미지를 Base64 데이터에서 저장: {visualization_path}")
        else:
            # Base64 데이터가 없는 경우 기존 방식으로 시도
            visualization_path = densepose_result.get("visualization_url")

            # 여전히 경로가 없거나 Windows 경로인 경우
            if not visualization_path or visualization_path.startswith('C:'):
                # 기본 이미지 생성 (빈 이미지 또는 로그 메시지)
                pose_pil = Image.new('RGB', (384, 512), (0, 0, 0))
                visualization_path = os.path.join(settings.OUTPUT_DIR, "densepose", f"{densepose_id}.png")
                pose_pil.save(visualization_path)
                print(f"DensePose Base64 데이터 또는 유효한 경로가 없어 기본 이미지 생성: {visualization_path}")
            elif os.path.isabs(visualization_path) and os.path.exists(visualization_path):
                # 가능한 경우 절대 경로에서 복사
