...
# 서버 URL 설정
API_URL = "http://localhost:8011"  # 로컬 테스트용
    ...
    try:
        ...
        with open(image_path, 'rb') as img_file:
            ...
            if response.status_code == 200:
                ...
                # 비주얼라이제이션 경로 확인
                vis_path = result.get("densepose_data", {}).get("visualization_path")
                if vis_path and os.path.exists(vis_path):
                    print(f"DensePose 시각화 이미지 로드 성공: {vis_path}")

                    # PIL로 직접 로드하여 UV 시각화 이미지 준비
                    dp_img = Image.open(vis_path).convert("RGB")

                    # 결과 이미지 저장 (디버깅용)
                    result_dir = './result'
                    os.makedirs(result_dir, exist_ok=True)
                    dp_save_path = os.path.join(result_dir, f"densepose_{os.path.basename(image_path)}")
                    dp_img.save(dp_save_path)

                    # 결과에 직접 경로 추가
                    result["densepose_data"]["processed_visualization"] = dp_save_path

                    return result
                else:
                    print(f"DensePose 시각화 이미지를 찾을 수 없음: {vis_path}")
            else:
...
def generate_agnostic_image(person_image_path, parsing_result, keypoints_json_path, category):
    """
    Agnostic 이미지 생성 함수
    ...
    Args:
        ...
        keypoints_json_path: 미디어파이프 키포인트 결과 파일 경로
        category: 의류 카테고리
        
    Returns:
        dict: 결과 정보 (agnostic_path, mask_path, agnostic_id 등)
    """
    print("\n=== Agnostic 이미지 생성 중... ===")
    
    # 서비스 API 직접 호출 또는 API 엔드포인트 호출
    try:
        # 1. 파싱 결과 ID 가져오기
        ...
        agnostic_result = agnostic_service.generate_agnostic(
            ...
            parsing_json_path=os.path.join(settings.OUTPUT_DIR, "human_parsing", f"{parsing_id}_mask.json"),
            keypoints_json_path=keypoints_json_path,
            category=category
        )
        
        if agnostic_result["status"] == "success":
            print(f"Agnostic 이미지 생성 완료: {agnostic_result['agnostic_path']}")
            print(f"마스크 이미지 생성 완료: {agnostic_result['mask_path']}")
        else:
            print(f"Agnostic 이미지 생성 실패: {agnostic_result.get('message', '알 수 없는 오류')}")
            return None
...
def try_on_with_idm_vton(person_image_path, clothing_image_path, category, output_path=None, save_mask_only=False, skip_parsing=False):
    """
    ...
    for dir_path in [
        ...
        os.path.join(settings.OUTPUT_DIR, "agnostic"),  # Agnostic 디렉토리 추가
    ]:
    ...
    if not skip_parsing:
        ...
        # 가상의 파싱 결과 생성
        parsing_result = {"file_id": parsing_id, "mask_json_path": parsing_result_path}
    ...
    try:
        ...
        if mediapipe_response.status_code == 200:
            ...
            # 키포인트 JSON 저장
            keypoints_dir = os.path.join(settings.OUTPUT_DIR, "mediapipe")
            os.makedirs(keypoints_dir, exist_ok=True)
            keypoints_json_path = os.path.join(keypoints_dir, f"{parsing_id}_keypoints.json")
            
            with open(keypoints_json_path, 'w') as f:
                json.dump(mediapipe_result, f, indent=2)
            
            print(f"키포인트 JSON 저장 완료: {keypoints_json_path}")
    ...
    # 3. Agnostic 이미지 생성
    print("\n3. Agnostic 이미지 생성 중...")
    agnostic_result = None
    try:
        if parsing_result and keypoints_json_path:
            ...
            if agnostic_result:
                # Agnostic 이미지 및 마스크 저장 (결과 디렉토리에 복사)
                try:
                    from shutil import copyfile
                    agnostic_img_output = os.path.join(result_dir, "agnostic_image.jpg")
                    agnostic_mask_output = os.path.join(result_dir, "agnostic_mask.png")
                    
                    copyfile(agnostic_result["agnostic_path"], agnostic_img_output)
                    copyfile(agnostic_result["mask_path"], agnostic_mask_output)
                    
                    print(f"Agnostic 이미지 복사 완료: {agnostic_img_output}")
                    print(f"Agnostic 마스크 복사 완료: {agnostic_mask_output}")
                except Exception as e:
                    print(f"Agnostic 이미지 복사 실패: {str(e)}")
            else:
                print("Agnostic 이미지 생성 실패")
        else:
            print("파싱 결과 또는 키포인트 없음, Agnostic 이미지 생성 건너뜀")
    except Exception as e:
        print(f"Agnostic 이미지 생성 오류 (무시됨): {str(e)}")
    ...
    if save_mask_only:
        ...
        return {
            ...
            "category": category,
            "agnostic_id": agnostic_result["agnostic_id"] if agnostic_result else None
        }
    
    # 4. DensePose 정보 가져오기 (외부 API)
    print("\n4. DensePose 처리 중...")
    densepose_result = get_densepose(person_image_path)
    if not densepose_result or densepose_result.get('status') != 'success':
    ...
    if 'mediapipe_result' in locals() and mediapipe_result and mediapipe_result.get('pose_landmarks'):
    ...
    try_on_data = {
        ...
        "agnostic_id": agnostic_result["agnostic_id"] if agnostic_result else None  # Agnostic 이미지 ID 추가
    }
    
    response = requests.post(f"{API_URL}/api/tryon/idm-vton", json=try_on_data)
    
    if response.status_code != 200:
        print(f"가상 착용 처리 실패: {response.status_code}")
        print(response.text)
        return None
    
    result = response.json()
    print("IDM VTON 처리 완료")
    
    # 결과 이미지 저장
    final_output_path = output_path if output_path else os.path.join(result_dir, f"result_{category}.jpg")
    
    if 'image_url' in result:
        img_response = requests.get(f"{API_URL}{result['image_url']}")
        if img_response.status_code == 200:
            img = Image.open(io.BytesIO(img_response.content))
            img.save(final_output_path)
            print(f"\n결과 이미지 저장 완료: {final_output_path}")
            
            # 비교 이미지 생성 (원본 + 결과)
            try:
                person_img = Image.open(person_image_path)
                cloth_img = Image.open(clothing_image_path)
                result_img = img
                
                # 동일한 크기로 리사이즈
                size = (384, 512)  # (width, height)
                person_img = person_img.resize(size)
                cloth_img = cloth_img.resize(size)
                result_img = result_img.resize(size)
                
                # 가로로 이미지 합치기
                comparison = Image.new('RGB', (size[0] * 3, size[1]))
                comparison.paste(person_img, (0, 0))
                comparison.paste(cloth_img, (size[0], 0))
                comparison.paste(result_img, (size[0] * 2, 0))
                
                # 비교 이미지 저장
                comparison_path = os.path.join(result_dir, f"comparison_{category}.jpg")
                comparison.save(comparison_path)
                print(f"비교 이미지 저장 완료: {comparison_path}")
            except Exception as e:
                print(f"비교 이미지 생성 실패: {str(e)}")
    
    return result

def main():
    """메인 함수: 커맨드라인에서 실행할 때 사용"""
    parser = argparse.ArgumentParser(description='TryOnAPI 테스트')
    parser.add_argument('--person', type=str, default=r"C:\Users\tjdwn\OneDrive\Desktop\parsingData\data\1.Training\original\Model-Image\1008_A003_000.jpg", help='사용자 이미지 경로')
    parser.add_argument('--clothing', type=str,  default=r"C:\Users\tjdwn\OneDrive\Desktop\parsingData\data\2.Validation\original\Item-Image\1014004_F.jpg", help='의류 이미지 경로')
    parser.add_argument('--category', type=str, default="upper", choices=['upper', 'lower', 'dress', 'outer'],
                       help='의류 카테고리 (upper, lower, dress, outer)')
    parser.add_argument('--output', type=str, default= './result/output.jpg', help='결과 이미지 저장 경로')
    parser.add_argument('--mask-only', action='store_true', help='마스크 생성만 테스트 (가상 착용 생략)')
    parser.add_argument('--skip-parsing', action='store_true', default= False, help='인체 파싱 건너뛰기 (테스트 모드)')
    
    args = parser.parse_args()
    
    # 입력 파일 존재 확인
    if not os.path.exists(args.person):
        print(f"오류: 사용자 이미지 파일을 찾을 수 없습니다: {args.person}")
        return
        
    if not os.path.exists(args.clothing) and not args.mask_only:
        print(f"오류: 의류 이미지 파일을 찾을 수 없습니다: {args.clothing}")
        return
    
    print(f"사용자 이미지: {args.person}")
    if not args.mask_only:
        print(f"의류 이미지: {args.clothing}")
    print(f"카테고리: {args.category}")
    print(f"결과 저장 경로: {args.output}")
    if args.mask_only:
        print("모드: 마스크 생성 테스트 (가상 착용 생략)")
    if args.skip_parsing:
        print("모드: 인체 파싱 건너뛰기 (테스트 모드)")
    
    # IDM VTON으로 가상 착용 또는 마스크 생성 시작
    result = try_on_with_idm_vton(
        person_image_path=args.person,
        clothing_image_path=args.clothing if not args.mask_only else args.person,  # 마스크 모드에서는 의류 경로 무시
    ...
    if result:
        if args.mask_only:
            print("\n=== 마스크 생성 테스트 성공! ===")
            # 마스크 이미지 시각화 (GUI 환경에서만 작동)
            mask_path = os.path.join(os.path.dirname(args.output), f"mask_{args.category}.png")
            overlay_path = os.path.join(os.path.dirname(args.output), f"overlay_{args.category}.jpg")
            agnostic_path = os.path.join(os.path.dirname(args.output), "agnostic_image.jpg")
            
            try:
                if os.path.exists(mask_path):
                    mask_img = Image.open(mask_path)
                    display_image(mask_img, f"{args.category} 마스크")
                
                if os.path.exists(overlay_path):
                    overlay_img = Image.open(overlay_path)
                    display_image(overlay_img, f"{args.category} 마스크 오버레이")
                    
                if os.path.exists(agnostic_path):
                    agnostic_img = Image.open(agnostic_path)
                    display_image(agnostic_img, "Agnostic 이미지")
            except Exception as e:
            ...
            # 결과 이미지 표시 (GUI 환경에서만 작동)
            if 'image_url' in result:
                try:
                    img_response = requests.get(f"{API_URL}{result['image_url']}")
                    if img_response.status_code == 200:
                        img = Image.open(io.BytesIO(img_response.content))
                        display_image(img, "가상 착용 결과")
                except Exception as e:
                    print(f"이미지 표시 오류: {str(e)}")
    else:
        print("\n=== 처리 실패 ===")
...
if __name__ == "__main__":
    # 커맨드라인 인자가 있으면 main(), 없으면 demo() 실행
    main()
